...
#[handler]
pub async fn register(req: Json<Register>, db: Data<&Arc<DatabaseConnection>>) -> Result<StatusCode, StatusCode> {
...
pub async fn login(req: Json<Login>, db: Data<&Arc<DatabaseConnection>>) -> Result<Response, StatusCode> {
    ...
    let account = queries::accounts::by_uuid_or_login(req.login.to_lowercase())
        .one(db)
        .await
